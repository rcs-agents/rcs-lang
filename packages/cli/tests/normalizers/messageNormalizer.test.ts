...
describe('MessageNormalizer', () => {
  ...
  describe('Suggestions', () => {
    ...
    it('should limit reply text to 25 characters', () => {
      ...
      expect(messages.LongReply.contentMessage.suggestions?.[0].reply?.text.length).toBeLessThanOrEqual(25);
    });
    ...
    it('should extract action suggestions with openUrlAction', () => {
      ...
      const suggestion = TestUtils.createSuggestionNode('action', 'Visit', 'visit_site', openUrlAction);
      const contentMessage = TestUtils.createMockNode('content_message', undefined, [
        TestUtils.createMockNode('suggestions', undefined, [suggestion])
      ]);
    ...
    it('should extract dial action suggestions', () => {
      ...
      const suggestion = TestUtils.createSuggestionNode('action', 'Call', 'call_support', dialAction);
      const contentMessage = TestUtils.createMockNode('content_message', undefined, [
        TestUtils.createMockNode('suggestions', undefined, [suggestion])
      ]);
  ...
  describe('Rich Cards', () => {
    it('should extract standalone rich cards', () => {
      ...
      expect(messages.RichCardMessage.contentMessage.richCard?.standaloneCard?.cardContent.title).toBe('Product Title');
      expect(messages.RichCardMessage.contentMessage.richCard?.standaloneCard?.cardContent.description).toBe('Product description');
    });
    ...
    it('should extract carousel rich cards', () => {
      ...
      expect(messages.CarouselMessage.contentMessage.richCard?.carouselCard?.cardContents).toHaveLength(2);
      expect(messages.CarouselMessage.contentMessage.richCard?.carouselCard?.cardWidth).toBe('MEDIUM');
    });
    ...
    it('should extract media from cards', () => {
      ...
      expect(messages.MediaMessage.contentMessage.richCard?.standaloneCard?.cardContent.media).toEqual({
        height: 'MEDIUM',
        contentInfo: {
  ...
  describe('Schema Compliance', () => {
    ...
    it('should ensure suggestions follow schema constraints', () => {
      ...
      expect(TestUtils.validateSuggestionsStructure(messages.SuggestionTest.contentMessage.suggestions || [])).toBe(true);
    });
  ...
  describe('Error Handling', () => {
    ...
    it('should generate valid postback data for all suggestions', () => {
      ...
      const postbackData = messages.PostbackTest.contentMessage.suggestions?.[0].reply?.postbackData;
