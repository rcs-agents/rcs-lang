...
import { IFileSystem, IFileStats } from '@rcl/core-interfaces';
...
export class MemoryFileSystem implements IFileSystem {
  ...
  async readFile(filePath: string, encoding?: string): Promise<Result<string>> {
  ...
  async writeFile(filePath: string, content: string, encoding?: string): Promise<Result<void>> {
    const normalizedPath = this.normalize(filePath);
  ...
  async mkdir(dirPath: string, recursive: boolean = false): Promise<Result<void>> {
    const normalizedPath = this.normalize(dirPath);
  ...
  async rmdir(dirPath: string, recursive: boolean = false): Promise<Result<void>> {
    const normalizedPath = this.normalize(dirPath);
    ...
    const prefix = normalizedPath === '/' ? '/' : normalizedPath + '/';
    const hasContents = Array.from(this.files.keys()).some(f => f.startsWith(prefix)) ||
