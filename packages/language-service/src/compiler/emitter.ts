...
import { CompiledAgent, Diagnostic } from '../program/types';
import { RclConfig } from '../config/types';
import { getOutputPath } from '../config/loader';
...
export class Emitter {
   ...
   */
  private async emitJavaScript(agent: CompiledAgent, outputPath: string, baseName: string): Promise<void> {
    const dir = path.dirname(outputPath);
    await fs.promises.mkdir(dir, { recursive: true });
   ...
   */
  private generateESMJavaScript(agent: CompiledAgent, jsonFileName: string): string {
    return `// Generated by RCL Compiler
// This file contains the compiled output from your RCL agent definition

import agentData from './${jsonFileName}' assert { type: 'json' };
...
`;
   ...
   */
  private generateCommonJSJavaScript(agent: CompiledAgent, jsonFileName: string): string {
    return `// Generated by RCL Compiler
// This file contains the compiled output from your RCL agent definition
...
`;
   ...
   */
  private async emitDeclarations(agent: CompiledAgent, outputPath: string, baseName: string): Promise<void> {
    const dir = path.dirname(outputPath);
    await fs.promises.mkdir(dir, { recursive: true });
