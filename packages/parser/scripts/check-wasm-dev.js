...
const isDev = process.env.RCL_DEV_MODE === 'true' || process.argv.includes('--dev');
...
// In development mode, allow running without WASM
if (isDev) {
  console.warn('\n⚠️  Running in development mode without WASM file');
  console.warn('The parser will use a mock implementation.');
  console.warn('To test with the real parser, build the WASM file using one of the methods below.\n');
  
  // Set environment variable for the parser to know it should use mock
  process.env.RCL_USE_MOCK_PARSER = 'true';
  process.exit(0);
}
...
console.error('');
console.error('Option 5: Run in development mode');
console.error('  RCL_DEV_MODE=true npm test');
console.error('  This will use a mock parser for basic testing');
console.error('');
