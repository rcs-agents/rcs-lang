...
import { type IParser, type ParserConfig, IParseResult } from '@rcl/core';
...
export class ParserFactory {
  ...
  static async create(config: ParserConfig = {}): Promise<Result<IParser>> {
    const platform = config.platform || ParserFactory.detectPlatform();
    
    try {
      switch (platform) {
        case 'node':
          return await ParserFactory.createNodeParser(config);
        
        case 'browser':
          return await ParserFactory.createWasmParser(config);
        
        case 'universal': {
          // Try Node first, fall back to WASM
          const nodeResult = await ParserFactory.createNodeParser(config);
