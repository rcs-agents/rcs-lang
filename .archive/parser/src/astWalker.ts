import { type RCLASTNode, RCLNode } from './astTypes';

export type ASTVisitor = (node: RCLASTNode, depth: number, parent?: RCLASTNode) => boolean | undefined;
export type SimpleVisitor = (node: RCLASTNode) => boolean | undefined;

// Convenience function for simple walking
export function walkAST(node: RCLASTNode | null, visitor: SimpleVisitor): void {
  if (!node) return;

  const walkNode = (n: RCLASTNode): boolean | undefined => {
...
export class ASTWalker {
  ...
  public static walk(node: RCLASTNode | null, visitor: ASTVisitor): void {
    if (!node) return;
    ASTWalker.walkRecursive(node, visitor, 0);
  }
  ...
  private static walkRecursive(
    ...
    if (shouldContinue !== false && 'children' in node && node.children) {
      for (const child of node.children) {
        ASTWalker.walkRecursive(child as RCLASTNode, visitor, depth + 1, node);
      }
    }
  }
  ...
  public static getDescendants(node: RCLASTNode): RCLASTNode[] {
    ...
    ASTWalker.walk(node, (child, depth) => {
      if (depth > 0) {
  ...
  public static findLowestCommonAncestor(node1: RCLASTNode, node2: RCLASTNode): RCLASTNode | null {
    const ancestors1 = ASTWalker.getAncestors(node1);
    const ancestors2 = ASTWalker.getAncestors(node2);
  ...
  public static findDeepestNodeAtPosition(
    ...
    ASTWalker.walk(root, (node, depth) => {
      if (ASTWalker.containsPosition(node, line, character) && depth > maxDepth) {
        deepestNode = node;
        maxDepth = depth;
      }
    });
