// RCL Interactive Diagram - Complete Implementation
// Enhanced with node dragging, proper arrows, and text handling
...
(() => {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    console.log('🚀 Interactive Diagram: DOMContentLoaded');
  ...
  function setupMessageHandlers() {
    console.log('🔧 Interactive Diagram: Setting up message handlers');

    window.addEventListener('message', (event) => {
      const message = event.data;

      console.log('📨 Interactive Diagram: Received message:', {
        type: message.type,
  ...
  function initializeDiagram() {
    console.log('🎨 Interactive Diagram: Initializing');
    ...
    console.log('✅ Interactive Diagram: SVG created and appended');
  }
  ...
  function makeNodeDraggable(nodeElement, node) {
    ...
    handle.addEventListener('mousedown', (e) => {
      ...
      // Select the node
      currentState.selectedNodes.clear();
      currentState.selectedNodes.add(node.id);
      updateSelections();
  ...
  function renderNode(node) {
    ...
    switch (node.type) {
      case 'start': {
        // Small green circle for pure start
      ...
      case 'rich_card': {
        // Star shape for rich cards
        shape = document.createElementNS('http://www.w3.org/2000/svg', 'path');
        const starPath = createStarPath(50, 25, 20, 10, 5);
  ...
  function renderMatchNode(node) {
    ...
    for (const [index, option] of options.entries()) {
      ...
      port.setAttribute('cx', width - 5);
      port.setAttribute('cy', y + optionHeight / 2 - 2.5);
      ...
      port.setAttribute('data-port', `option-${index}`);
      port.style.cursor = 'crosshair';
      matchGroup.appendChild(port);
    }
  ...
  function createConnectionPorts(width, height) {
    ...
    for (const pos of positions) {
      ...
      port.addEventListener('mouseenter', () => {
        if (currentState.connectionMode || currentState.draggedConnection) {
  ...
  function renderEdge(edge, nodes) {
    ...
    edgeGroup.addEventListener('click', (e) => {
      e.stopPropagation();
      currentState.selectedEdges.clear();
      currentState.selectedEdges.add(edge.id);
      updateSelections();
    });
  ...
  function calculateEdgePoints(source, target, edge) {
    // Get node centers
    let sx = source.position.x + (source.width || 100) / 2;
    let sy = source.position.y + (source.height || 50) / 2;
    const tx = target.position.x + (target.width || 100) / 2;
    const ty = target.position.y + (target.height || 50) / 2;

    // Handle match node ports
    if (source.type === 'match' && edge.sourcePort) {
      const portIndex = Number.parseInt(edge.sourcePort.split('-')[1]);
      if (!Number.isNaN(portIndex)) {
        sy = source.position.y + 30 + portIndex * 25 + 12.5;
        sx = source.position.x + source.width;
      }
    }
