...
interface TransitionJSON {
  ...
  target?: string;       // Target reference using type:ID format (state:Name, flow:Name, @variable, :ok/:cancel/:error)
  ...
  flowInvocation?: {     // Flow invocation with result handling
    ...
    onResult: {          // Handlers for different flow outcomes
      ok?: {
        operations?: Array<Operation>;  // Operations before transitioning
